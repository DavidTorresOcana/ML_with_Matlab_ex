...
% You need to return the following variables correctly 
    ...
    for i=1:m
       ...
       % Backward
        delta3=a3(:,i) - y(i,:)';
        delta2=Theta2'*delta3.*(a2.*(ones(size(a2))-a2));% No tiene sentido la primera diferencia pues es la diferencia con 1
        Delta1=Delta1+delta2(2:end)*a1';
        Delta2=Delta2+delta3*a2';
    end 

    J = -1/m*( sum( sum(y.*log( a3' )) )  + sum( sum(( ones(size(y))-y ).*log(ones(size(a3'))-a3') ) )   )   + lambda/(2*m)*( sum(sum(Theta1(:,2:end).^2 )) + sum(sum(Theta2(:,2:end).^2 ))   );
