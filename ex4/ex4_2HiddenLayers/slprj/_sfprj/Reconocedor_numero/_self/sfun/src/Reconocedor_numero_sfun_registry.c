...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_Reconocedor_numero_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Reconocedor_numero_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_Reconocedor_numero_autoinheritance_info( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "Reconocedor_numero") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Reconocedor_numero_autoinheritance_info(nlhs,plhs,2,newRhs);
  }
...
{
  extern unsigned int sf_Reconocedor_numero_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "Reconocedor_numero") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Reconocedor_numero_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
extern unsigned int sf_Reconocedor_numero_method_dispatcher(SimStruct *S,
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Reconocedor_numero")) {
    return(sf_Reconocedor_numero_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void Reconocedor_numero_terminator(void);
...
extern void Reconocedor_numero_initializer(void);
extern void Reconocedor_numero_register_exported_symbols(SimStruct* S);
extern void Reconocedor_numero_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  Reconocedor_numero_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      Reconocedor_numero_debug_initialize();
    }

    Reconocedor_numero_initializer();
  }
