...
% You need to return the following variables correctly 
    ...
    for i=1:m
       ...
       % Backward
        delta3(:,i)=a3(:,i) - y(i,:)';
        delta2=Theta2'*delta3(:,i).*(a2.*(ones(size(a2))-a2)); % No tiene sentido la primera diferencia pues es la diferencia con 1
    ...
    J = 0.5/m*(  sum( sum(    delta3.^2   ) ) )      + lambda/(2*m)*( sum(sum(Theta1(:,2:end).^2 )) + sum(sum(Theta2(:,2:end).^2 ))   );
