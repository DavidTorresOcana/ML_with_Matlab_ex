...
% You need to return the following variables correctly 
    ...
    for i=1:m
       ...
       % Backward
        delta4(:,i)=a4(:,i) - y(i,:)';
        delta3=Theta3'*delta4(:,i);
        delta2=Theta2'*delta3(2:end); % No tiene sentido la primera diferencia pues es la diferencia con 1
        ...
        Delta3=Delta3+delta4(:,i)*a3';

    end 
    J = 0.5/m*(  sum( sum(    delta4.^2    ) ) )     + lambda/(2*m)*( sum(sum(Theta1(:,2:end).^2 )) + sum(sum(Theta2(:,2:end).^2 ))+sum(sum(Theta3(:,2:end).^2 ))   );
